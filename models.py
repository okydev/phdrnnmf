...
import math
import numpy as np
from text_analysis.rnn_model import RNN_module
...
def PHDMF(res_dir, train_user, train_item, valid_user, test_user,
    ...
    frmse = open(res_dir + '/rmse.csv', 'w')
    frmse.write("Loss,Time,Convergence,Tr_eval,Val_eval,Te_eval\n")
    frmse.flush()
    ...
    rnn_module = RNN_module(dimension, vocab_size, dropout_rate,emb_dim, max_len, num_kernel_per_ws, init_W)
    theta = rnn_module.get_projection_layer(CNN_X)
    ...
    for iteration in range(max_iter):
        ...
        history = rnn_module.train(CNN_X, V, item_weight, seed)
        theta = rnn_module.get_projection_layer(CNN_X)
        ...
        if (val_eval < pre_val_eval):
            rnn_module.save_model(res_dir + '/CNN_weights.hdf5')
        ...
        f1.write("Loss: %.5f Elpased: %.4fs Converge: %.6f Tr: %.5f Val: %.5f Te: %.5f\n" % (
            loss, elapsed, converge, tr_eval, val_eval, te_eval))
        frmse.write("%.5f,%.4fs,%.6f,%.5f,%.5f,%.5f\n" % (
            loss, elapsed, converge, tr_eval, val_eval, te_eval))
        frmse.flush()
